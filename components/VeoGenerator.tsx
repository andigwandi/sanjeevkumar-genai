...
export const VeoGenerator: React.FC = () => {
  ...
  const generateVideo = async () => {
    ...
    setLoading(true);
    setVideoUrl(null);
    setStatus('Submitting video generation request to server...');
    ...
    try {
      ...
      const res = await fetch('/api/ai/veo', {
        ...
        body: JSON.stringify({ base64Data, prompt, aspectRatio })
      });

      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || 'Failed to render motion video.');
      }
      ...
      console.error(error);
      alert(error.message || "Video generation encountered an error. Please try again.");
    } finally {
      setLoading(false);
      setStatus('');
    }
